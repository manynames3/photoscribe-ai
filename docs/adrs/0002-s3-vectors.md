...
The project needs vector search for photo descriptions, but a managed OpenSearch Serverless collection would be expensive for a low-volume portfolio preview. The data model is simple: one vector per photo, plus metadata filters.
...
- S3 Vectors is a better fit for this preview than for complex analytics or multi-tenant search features.
